...
def init_db():
    ...
    """)
    # Backward-compatible migration: older databases created before
    # per-user history was added won't have a user_id column yet.
...
# Real feature importances extracted from the trained RandomForest model,
# used to power the chart on the About page.
FEATURE_IMPORTANCE = sorted(
    zip(FEATURES, getattr(model, "feature_importances_", [0] * len(FEATURES))),
...
def save_prediction(user_id, raw_inputs, prediction, confidence):
    db = get_db()
    cols = ["ts", "user_id"] + FEATURES + ["prediction", "confidence"]
    placeholders = ", ".join("?" for _ in cols)
    values = (
        [datetime.now().strftime("%Y-%m-%d %H:%M"), user_id]
        + [raw_inputs[f] for f in FEATURES]
...
def fetch_history(user_id, limit=30):
    db = get_db()
    rows = db.execute(
        "SELECT * FROM predictions WHERE user_id = ? ORDER BY id DESC LIMIT ?", (user_id, limit)
    ).fetchall()
...
def get_user_stats(user_id):
    db = get_db()
    rows = db.execute(
        "SELECT prediction, COUNT(*) as n FROM predictions WHERE user_id = ? GROUP BY prediction",
        (user_id,),
    ).fetchall()
    counts = {"Low": 0, "Medium": 0, "High": 0}
    for r in rows:
        if r["prediction"] in counts:
            counts[r["prediction"]] = r["n"]
...
def about():
    ...
    return render_template(
        "about.html", active="about", stats=stats, feature_importance=FEATURE_IMPORTANCE
    )
...
def predict():
    ...
    try:
        ...
        save_prediction(session["user_id"], raw_inputs, prediction, confidence)
